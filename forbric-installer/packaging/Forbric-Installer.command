...
# carries its version (forbric-installer-0.1.0.jar).
#
...
JAR=""
for candidate in "$DIR"/forbric-installer*.jar; do
	[ -f "$candidate" ] && JAR="$candidate"
done
if [ -z "$JAR" ]; then
	echo "Could not find forbric-installer*.jar next to this script." >&2
