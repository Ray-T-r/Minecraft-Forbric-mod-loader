#!/usr/bin/env bash
# Builds run/canary/forbricinteraction.jar for gate-m44: a NeoForge mod that uses items on blocks and picks blocks with a
# fake player while listening through the unmodified fabric-events-interaction-v0 and NeoForge's UseItemOnBlockEvent.
...
compile_game = pathlib.Path(os.environ.get('M44_COMPILE_GAME', old / 'run/neoforge-patched/patched-mc-neoforge-26.2.jar'))
...
fapi = pathlib.Path(os.environ.get('M44_FABRIC_API', kernel / 'run/client-merged-pack/mods/fabric-api-0.155.2+26.2.jar'))
libraries = []
...
modules = kernel / 'run/canary/m44-modules'; modules.mkdir(parents=True, exist_ok=True)
selected = []
...
for path in [compile_game, forge, neo, fapi]:
    if not path.is_file(): raise SystemExit(f'M44 prerequisite absent: {path}')
root = kernel / 'canary/interaction-events'
...
staged = work / 'forbricinteraction.jar'
