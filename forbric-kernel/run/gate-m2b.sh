...
check "access wideners merged + applied"       "merged [0-9]+ class tweaker\(s\).*target class" "$LOG"
check "MC libraries owned by the game loader"  "[0-9]{2,} MC library jar\(s\)" "$LOG"

step "mods actually ran (must PASS)"
check "Fabric main + side entrypoints invoked" "invoked [0-9]+ Fabric main entrypoint\(s\) \+ [0-9]+ server" "$LOG"
